...
import { ConfluenceService, ConfluenceWebhookEvent } from "../services/confluenceService";

export async function confluencewebhook(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    try {
        ...
        if (request.method === 'POST') {
            ...
            if (!webhookEvent.eventType) {
                return {
                    status: 400,
                    body: JSON.stringify({ error: "Missing eventType in webhook data" }),
                    headers: { 'Content-Type': 'application/json' }
                };
            }
            ...
            // 处理webhook事件
            try {
                const result = await confluenceService.handleWebhookEvent(webhookEvent);
